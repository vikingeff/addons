	...
	["DungeonList"] = {
		[L["Mists of Pandaria"]] = {
			[15164] = {                                     
				...
				-- Throne of Thunder
				[98] = "LFR Throne of Thunder",
				[99] = "Normal Throne of Thunder",
				[100] = "Heroic Throne of Thunder"
			},
		},
		[L["Cataclysm"]]  = {
			[15096] = {
...
--[=[	[L["Wrath of the Lich King"]]  = {
		...
		},--]=] -- we no longer care about WotLK raids, do we? 
	},
	ColorArray = {
		[0] = { .55, .55, .55  },
		[1] = { .55, .55, .55 },
...
f.Help = {
	...
	["Answers"] = {
		...
		[L["What happened to the database?"]] = L["In GearScore 3.0 the database stored a vast array of information about a player to allow for the remote armory feature. This information required an unreasonable amount of system resources and generally slowed a player's computer down. In PlayerScore 4.0 the remote armory feature was removed for this reason, you can still inspect any player to get all the information."],
	},
};
...
function f:runSettings()
	...
	-- bonus tab is also removed since it only contains links to the now defunct playerscore.com website
	TenTonHammer_Settings["NewsTab"] = false
...
function f:CHAT_MSG_ADDON(eventName, prefix, message, channel, sender)
	...
	if ( prefix == "GSY_Version" ) and  ( tonumber(message) ) and ( tonumber(message) > f.data["Version"] ) then
		...
		print("|cffff0000PlayerScore|r The newest version is now available!");
...
function f:BuildRaidList()
	...
	f.data["RaidOrder"] = {
		[L["Mists of Pandaria"]] = {
			...
			L["LFR Throne of Thunder"],
			L["Normal Throne of Thunder"],
			L["Heroic Throne of Thunder"],
		},
		[L["Cataclysm"]] = {
			L["Dragon Soul"], 
			L["Firelands"],
...
function f:ScanEntireParty()
	...
	local Group = "party";
	local MaxGroup = GetNumGroupMembers();
...
function f:displayDatabaseRaid()
	...
	for i = 1,25 do
		local Temp = "party"..i;
		if ( ( "party"..i == "party"..(GetNumGroupMembers() + 1) ) and not ( UnitName("raid"..i) ) ) then
			Temp = "player";
		end;
		if ( UnitName("raid"..i) or UnitName(Temp) ) then
			local Target = "raid"..i;
			if not ( UnitName(Target) ) then Target = "party"..i; end;
			if ( Target == "party"..(GetNumGroupMembers() + 1) ) then
				Target = "player";
			end;
			local PlayerName, PlayerRealm = UnitName(Target);
			if ( f.PartyData[PlayerName] ) then
