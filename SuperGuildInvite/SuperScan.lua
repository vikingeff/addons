...
-- Prevent the Friends/who frame from
-- showing while scanning
--[[
local oldShow = FriendsFrame.Show;
local CanShowFriendsFrame = true;
local function ModifiedFriendsShow(...)
	if (not CanShowFriendsFrame) then
		return;
	else
		oldShow(FriendsFrame,...);
	end
end

local function AllowFriendsFrame(allow)
	CanShowFriendsFrame = allow;
	FriendsFrame.Show = ModifiedFriendsShow;
end

FriendsFrame.Show = ModifiedFriendsShow;
]]

local oldShowUIPanel = ShowUIPanel;
function ShowUIPanel(frame, force)
	if (shouldHideFriendsFrame and frame == FriendsFrame) then
		return;
	end
	oldShowUIPanel(frame, force);
end

local function AllowFriendsFrame(allow)
	
	shouldHideFriendsFrame = not allow;
